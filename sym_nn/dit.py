...
class PositionEmbedder:
    ...
    def __init__(self, hidden_size, scale, max_period=10000):
        self.hidden_size = hidden_size
        self.scale = scale
        self.max_period = max_period

    @staticmethod
    def positional_embedding(x, dim, scale=1., max_period=10000):
        # x: [N, B, C]
        in_dim = x.shape[-1]
        m = dim // (2 * in_dim)
        remainder = dim % (2 * in_dim)
        freqs = torch.exp(
            -math.log(max_period) * torch.arange(start=0, end=m, dtype=torch.float32) / m  # [m]
        ).to(device=x.device)
        args = scale * freqs * torch.unsqueeze(x, -1).float()
        embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1).flatten(-2, -1)
        if remainder:
            embedding = torch.cat([embedding, torch.zeros_like(embedding[:, :, :remainder])], dim=-1)
        return embedding

    def apply(self, x):
        x_emb = self.positional_embedding(x, self.hidden_size, scale=self.scale, 
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, mlp_dropout=0, mlp_type="mlp", **block_kwargs):
        ...
        print("The type of our mlp is", mlp_type)
